...
class SawyerPickPlaceWallEnvV2(SawyerXYZEnv):
    ...
    def compute_reward(self, action, obs):
        ...
        lift_gate = tcp_to_obj < 0.02 and (tcp_opened > 0) and (obj[2] - 0.015 > self.obj_init_pos[2])
        wall_passed_gate = obj[1] > 0.75
        done_gate = 1. * (obj_to_target < _TARGET_RADIUS)
        feature_dict = {
            "H_GM": in_place_and_object_grasped * (1 - done_gate),
            "L": 1. * lift_gate * (1 - done_gate),
            "I_mid_L_noPW": 4. * lift_gate * (not wall_passed_gate) * in_place_part1 * (1 - done_gate),
            "L_PW": 4. * lift_gate * wall_passed_gate * (1 - done_gate),
            "I_goal_L_PW": 3. * lift_gate * wall_passed_gate * in_place_part2 * (1 - done_gate),
            "S": 10. * done_gate,
        }
        reward = in_place_and_object_grasped + \
            lift_gate * in_place_and_object_grasped + \
            1. * lift_gate + \
            4. * lift_gate * (not wall_passed_gate) * in_place_part1 + \
            4. * lift_gate * wall_passed_gate + \
            3. * lift_gate * wall_passed_gate * in_place_part2
