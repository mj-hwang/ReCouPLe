...
class SawyerPushWallEnvV2(SawyerXYZEnv):
    ...
    def compute_reward(self, action, obs):
        ...
        touch_gate = tcp_to_obj < 0.02 and tcp_opened > 0
        wall_passed_gate = obj[1] > 0.75
        done_gate = 1. * (obj_to_target < self.TARGET_RADIUS)

        feature_dict = {
            "G": 2 * object_grasped * (1 - done_gate),
            "T": 1. * touch_gate * (1 - done_gate),
            "I_mid_T": 4. * touch_gate * (not wall_passed_gate) * in_place_part1 * (1 - done_gate),
            "T_PW":  4. * touch_gate * wall_passed_gate * (1 - done_gate),
            "I_goal_T_PW": 3. * touch_gate * wall_passed_gate * in_place_part2 * (1 - done_gate),
            "S": 10. * done_gate,
        }

        reward = 2 * object_grasped + \
            1. * touch_gate + \
            4. * touch_gate * (not wall_passed_gate) * in_place_part1 + \
            4. * touch_gate * wall_passed_gate + \
            3. * touch_gate * wall_passed_gate * in_place_part2
