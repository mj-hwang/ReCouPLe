...
class SawyerPushEnvV2(SawyerXYZEnv):
    ...
    def compute_reward(self, action, obs):
        ...
        done_gate = 1. * (target_to_obj < self.TARGET_RADIUS)
        feature_dict = {
            "G": 2 * object_grasped * (1 - done_gate),
            "T": 1. * touch_gate * (1 - done_gate),
            "G_T": 2. * touch_gate * object_grasped * (1 - done_gate),
            "I_goal_T": 5. * touch_gate * in_place * (1 - done_gate),
            "S": 10. * done_gate,
        }
        reward = 2 * object_grasped + \
            1. * touch_gate + \
            2. * touch_gate * object_grasped + \
            5. * touch_gate * in_place
