...
class SawyerPickPlaceEnvV2(SawyerXYZEnv):
    ...
    def evaluate_state(self, obs, action):
        ...
        reward, tcp_to_obj, tcp_open, obj_to_target, grasp_reward, in_place_reward, feature_dict = self.compute_reward(action, obs)
    ...
    def compute_reward(self, action, obs):
        ...
        lift_gate = tcp_to_obj < 0.02 and (tcp_opened > 0) and (obj[2] - 0.01 > self.obj_init_pos[2])
        done_gate = 1. * (obj_to_target < _TARGET_RADIUS)
        feature_dict = {
            "H_GG": in_place_and_object_grasped * (1 - done_gate),
            "L": 1. * lift_gate * (1 - done_gate),
            "I_goal_L": 5. * lift_gate * in_place * (1 - done_gate),
            "S": 10. * done_gate,
        }
        reward = in_place_and_object_grasped + \
            1. * lift_gate + \
            5. * lift_gate * in_place

        if obj_to_target < _TARGET_RADIUS:
            reward = 10.
        return [reward, tcp_to_obj, tcp_opened, obj_to_target, object_grasped, in_place, feature_dict]
