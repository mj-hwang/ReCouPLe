...
@register_env("PlaceSmallerCube-v1", max_episode_steps=50)
@register_env("PlaceSmallerCubeSwapped-v1", max_episode_steps=50, larger_cube_color="red")
class PlaceSmallerCubeEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def evaluate(self):
        is_obj_placed = (
            torch.linalg.norm(self.bin.pose.p - self.smaller_cube.pose.p, axis=1)
            <= self.goal_thresh
        )
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        obj_to_goal_dist = torch.linalg.norm(
            self.bin.pose.p - self.smaller_cube.pose.p, axis=1
        )
