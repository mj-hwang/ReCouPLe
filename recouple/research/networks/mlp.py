    ...
    def __init__(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            self.repr_dim = self.mlp(sample).shape[1]
...
class ContinuousMLPEncoder(nn.Module):
    def __init__(
        self,
        observation_space: gym.Space,
        action_space: gym.Space,
        ...
        **kwargs,
        ...
        if self.ensemble_size > 1:
            self.mlp = EnsembleMLP(input_dim, embedding_dim, ensemble_size=ensemble_size, **kwargs)
        else:
            self.mlp = MLP(input_dim, embedding_dim, **kwargs)
    ...
    def forward(self, obs: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        embedding = self.mlp(x).squeeze(-1)  # Remove the last dim
        if self.ensemble_size == 1:
            embedding = embedding.unsqueeze(0)  # add in the ensemble dim
        return embedding
...
class MLPLCValue(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ensemble_size = ensemble_size
        if self.ensemble_size > 1:
            self.mlp = EnsembleMLP(observation_space.shape[0] + embedding_dim, 1, ensemble_size=ensemble_size, **kwargs)
        else:
            self.mlp = MLP(observation_space.shape[0] + embedding_dim, 1, **kwargs)
        self.ortho_init = ortho_init
        self.output_gain = output_gain
        self.reset_parameters()
    ...
    def forward(self, obs, task_embedding):
        x = torch.cat((obs, task_embedding), dim=-1)
        v = self.mlp(x).squeeze(-1)  # Remove the last dim
        if self.ensemble_size == 1:
            v = v.unsqueeze(0)  # add in the ensemble dim
        return v
...
class ContinuousMLPLCCritic(nn.Module):
    def __init__(
        self,
        observation_space: gym.Space,
        action_space: gym.Space,
        ...
        **kwargs,
        ...
        assert isinstance(observation_space, gym.spaces.Box) and len(observation_space.shape) == 1
        self.ensemble_size = ensemble_size
        input_dim = observation_space.shape[0] + action_space.shape[0] + embedding_dim
        if self.ensemble_size > 1:
            self.q = EnsembleMLP(input_dim, 1, ensemble_size=ensemble_size, **kwargs)
        else:
            self.q = MLP(input_dim, 1, **kwargs)
    ...
    def forward(self, obs: torch.Tensor, action: torch.Tensor, task_embedding: torch.Tensor) -> torch.Tensor:
        x = torch.cat((obs, action, task_embedding), dim=-1)
        q = self.q(x).squeeze(-1)  # Remove the last dim
        if self.ensemble_size == 1:
            q = q.unsqueeze(0)  # add in the ensemble dim
        return q
...
class ContinuousMLPLCActor(nn.Module):
    def __init__(
        self,
        observation_space: gym.Space,
        action_space: gym.Space,
        ...
        **kwargs,
        ...
        self.mlp = MLP(observation_space.shape[0] + embedding_dim, action_space.shape[0], **kwargs)
        self.ortho_init = ortho_init
        self.output_gain = output_gain
        self.reset_parameters()
    ...
    def forward(self, obs: torch.Tensor, task_embedding: torch.Tensor) -> torch.Tensor:
        x = torch.cat((obs, task_embedding), dim=-1)
        return self.mlp(x)
