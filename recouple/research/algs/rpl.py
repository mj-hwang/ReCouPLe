...
#             rv1 = self.network.reward.get_reason_value(obs_1, action_1, reason_tokens, reason_masks).sum(dim=-1)
#             rv2 = self.network.reward.get_reason_value(obs_2, action_2, reason_tokens, reason_masks).sum(dim=-1)
...
#             rv_loss = self.reward_criterion(rv_logits, labels).mean()

#             # CAN'T USE torch.max!! see: https://stackoverflow.com/questions/55545354/differences-between-f-relux-and-torch-maxx-0
#             # alignment_value = self.network.reward.get_reason_task_alignment(reason_tokens, reason_masks, task_tokens, task_masks)
#             # alignment_loss = F.relu(-alignment_value + self.alignment_margin).mean()

#             with torch.no_grad():
#                 reward_accuracy = ((r2 > r1) == torch.round(labels)).float().mean()
#                 rv_accuracy = ((rv2 > rv1) == torch.round(labels)).float().mean()

#                 # belows are for debugging purposes
#                 mean_t1_embedding_norm = torch.norm(self.network.reward.get_traj_embedding(obs_1, action_1), dim=-1).mean()
#                 mean_t2_embedding_norm = torch.norm(self.network.reward.get_traj_embedding(obs_2, action_2), dim=-1).mean()

#             loss = rv_loss + self.task_loss_weight * reward_loss # + self.alignment_loss_weight * alignment_loss
#             # loss = rv_loss # + reward_loss
...
#             metrics["rv_loss"] = rv_loss.item()
#             metrics["rv_accuracy"] = rv_accuracy.item()
#             metrics["mean_T1_embedding_norm"] = mean_t1_embedding_norm.item()
#             metrics["mean_T2_embedding_norm"] = mean_t2_embedding_norm.item()
#             # metrics["alignment_loss"] = alignment_loss.item()
...
class RPL(OffPolicyAlgorithm):
    ...
    def train_step(self, batch: Dict, step: int, total_steps: int) -> Dict:
        ...
        reason_embeddings = torch.cat([batch["reason_embeddings"], batch["reason_embeddings"]], dim=0)  # (B, S+1)
        ...
        if step < self.reward_steps:
            ...
            # reason value
            reason_value = torch.einsum('obtd,bd->obt', traj_embeddings, reason_embeddings)
            rv1, rv2 = torch.chunk(reason_value.sum(dim=-1), 2, dim=1)  # Should return two (E, B)
            rv_logits = rv2 - rv1
            assert labels.shape == rv_logits.shape
            rv_loss = self.reward_criterion(rv_logits, labels).mean()
